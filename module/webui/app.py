...
class AlasGUI(Frame):
    ...
    def set_aside(self) -> None:
        ...
        (
            put_icon_buttons(
                Icon.ADD,
                buttons=[{'label': t('Gui.Aside.AddAlas'), 'value': 'AddAlas', 'color': 'aside'}],
    ...
    def alas_update_overview_task(self) -> None:
        ...
        if self.scope_expired_then_add('pending_task', [alive, self.alas_config.pending_task]):
            clear('running_tasks')
            clear('pending_tasks')
            clear('waiting_tasks')
            ...
            def put(name=None, origin=None):
                ...
                (
                    put_select(
                        ...
                        value=origin or 'template-bd2as',
                        scope=s,
                    ),
                )
    ...
    def show(self) -> None:
        ...
        with use_scope('content'):
            ...
            put_markdown(
                """
            BD2AS is a free open source software, if you paid for BD2AS from any channel, please refund.
            BD2AS 是一款免费开源软件，如果你在任何渠道付费购买了BD2AS，请退款。
            Project repository 项目地址：`https://github.com/megumiss/BrownDust2AutoScript`
            """
            ).style('text-align: center')
